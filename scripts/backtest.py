...
def run_backtest(symbol: str, initial_capital: float = 10000, 
    ...
    print(f"Start Date: {start_date or 'Default (6 months back)'}")
    print(f"End Date: {end_date or 'Today'}")
    print(f"Strategy: {strategy_name.upper()}")
    print()
    
    backtester = Backtester(initial_capital=initial_capital, strategy_name=strategy_name)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python backtest.py <SYMBOL> [start_date] [end_date] [strategy]")
        print("Example: python backtest.py AAPL 2024-01-01 2024-01-31 balanced")
    ...
    strategy_name = sys.argv[4] if len(sys.argv) > 4 else DEFAULT_STRATEGY
    
    run_backtest(symbol, initial_capital=10000, start_date=start_date, 
                end_date=end_date, strategy_name=strategy_name)
